...
enum BRX_ASSET_IMPORT_MORPH_TARGET_NAME : uint32_t
{
    ...
    //
    BRX_ASSET_IMPORT_MORPH_TARGET_NAME_INVALID = 0X7FFFFFFF
};
...
{
    BRX_ASSET_IMPORT_PHYSICS_CONSTRAINT_RAGDOLL = 0,
    BRX_ASSET_IMPORT_PHYSICS_CONSTRAINT_6DOF = 1
};
...
public:
    ...
    virtual bool get_is_double_sided() const = 0;

    // material
    // start with file:// : external file
    // start with data:// : internal data
    virtual uint8_t const *get_emissive_image_url_data() const = 0;
    virtual uint32_t get_emissive_image_url_size() const = 0;
    virtual brx_asset_import_vec3 get_emissive_factor() const = 0;
    virtual uint8_t const *get_normal_image_url_data() const = 0;
    virtual uint32_t get_normal_image_url_size() const = 0;
    virtual float get_normal_scale() const = 0;
    virtual uint8_t const *get_base_color_image_url_data() const = 0;
    virtual uint32_t get_base_color_image_url_size() const = 0;
    virtual brx_asset_import_vec4 get_base_color_factor() const = 0;
    virtual uint8_t const *get_metallic_roughness_image_url_data() const = 0;
    virtual uint32_t get_metallic_roughness_image_url_size() const = 0;
