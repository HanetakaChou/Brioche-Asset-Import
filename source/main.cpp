...
struct ui_controller_t
{
    ...
    int m_asset_import_get_open_file_name_file_type_index;
};
...
int main(int argc, char *argv[])
{
    ...
    {
        xcb_void_cookie_t cookie_change_property_wm_name = xcb_change_property(connection, XCB_PROP_MODE_REPLACE, window, XCB_ATOM_WM_NAME, XCB_ATOM_STRING, 8 * sizeof(uint8_t), 13, "Brioche ImGui");
...
static bool inline _internal_asset_import_get_open_file_name(void *platform_context, int &inout_file_type_index, mcrt_string *out_file_name, uint64_t *out_file_time_stamp);
...
int APIENTRY wWinMain(_In_ HINSTANCE hInstance, _In_opt_ HINSTANCE, _In_ LPWSTR lpCmdLine, _In_ int nCmdShow)
{
    ...
    HWND window = NULL;
    {
...

        {
            WNDCLASSEXW const window_class_create_info = {
                ...
                NULL,
                L"Brioche-Asset-Import:0XFFFFFFFF",
                LoadIconW(NULL, IDI_APPLICATION),
            };
...

        {
            ...
            window = CreateWindowExW(dw_ex_style, MAKEINTATOM(window_class), L"Brioche Asset Import", dw_style, rect.left, rect.top, rect.right - rect.left, rect.bottom - rect.top, desktop_window, NULL, hInstance, &wsi_state);
            assert(NULL != window);
        }
        ...
        uint64_t file_time_stamp;
        if (_internal_asset_import_get_open_file_name(window, wsi_state.m_ui_controller.m_asset_import_get_open_file_name_file_type_index, &file_name, &file_time_stamp))
        {
            assert(!file_name.empty());
            assert(0U != file_time_stamp);
    ...
    while (wsi_state.m_running)
    {
        ...
        if (wsi_state.m_running && wsi_state.m_window_width > 0 && wsi_state.m_window_height > 0)
        {
            ...
            wsi_state.m_renderer->draw(interval_time, &wsi_state.m_ui_model, &wsi_state.m_user_camera_model, (wsi_state.m_ui_view ? ImGui::GetDrawData() : NULL));
        }
    }
...
static HRESULT STDMETHODCALLTYPE NoRegCoCreate(const __wchar_t *dllName, REFCLSID rclsid, REFIID riid, void **ppv)
{
    HMODULE dynamic_library;
    dynamic_library = GetModuleHandleW(dllName);
...
static bool inline _internal_asset_import_get_open_file_name(void *platform_context, int &inout_file_type_index, mcrt_string *out_file_name, uint64_t *out_file_time_stamp)
{
    mcrt_wstring file_name_utf16;
    {
        ...
        constexpr COMDLG_FILTERSPEC const filter_specs[] = {
            {L"All Files", L"*.*"},
            {L"MMD Model Data", L"*.pmx"},
            {L"MMD Motion Data", L"*.vmd"},
            {L"glTF Binary", L"*.glb;*.vrm"},
        ...
        HRESULT res_file_open_dialog_set_file_type_index = file_open_dialog->SetFileTypeIndex(std::min(std::max(1, (inout_file_type_index + 1)), 5));
    ...
    {
        bool file_time_stamp_found;
        {
            HANDLE file = CreateFileW(file_name_utf16.c_str(), FILE_GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
            if (INVALID_HANDLE_VALUE != file)
            {
                FILETIME last_access_time;
                BOOL res_get_file_time = GetFileTime(file, NULL, &last_access_time, NULL);
                if (FALSE != res_get_file_time)
                {
                    (*out_file_time_stamp) = ((static_cast<uint64_t>(last_access_time.dwHighDateTime) << 32ULL) | static_cast<uint64_t>(last_access_time.dwLowDateTime));
                    file_time_stamp_found = true;
                }
                else
                {
                    assert(0);
                    file_time_stamp_found = false;
                }
        ...
        if (file_time_stamp_found)
        {
            assert(0U != (*out_file_time_stamp));
...
static void inline ui_controller_init(ui_controller_t *ui_controller)
{
    ...
    ui_controller->m_asset_import_get_open_file_name_file_type_index = 4;
}

extern char const *const _internal_havok_pc_xs_license_string;
...
static void inline ui_simulate(void *platform_context, ui_model_t *ui_model, ui_controller_t *ui_controller)
{
    ...
    {
        constexpr char const *const labels[4] = {
            "Language",
            "言語",
            "語言",
            "语言"};

        constexpr char const *const items[4] = {
            "English",
            "日本語",
            "華語",
            "华语"};

        ImGui::Combo(labels[ui_controller->m_language_index], &ui_controller->m_language_index, items, IM_ARRAYSIZE(items));
    }
    ...
    {
        {
            constexpr char const *const label[4] = {
                "View##English",
                "見##Japanese",
                "視圖##Traditional-Chinese",
                "视图##Simplified-Chinese"};

            constexpr char const *const items[4][5] = {
                {"Scene Color", "GBuffer Normal", "GBuffer Roughness", "Linear Depth", "Reconstructed Linear Depth"},
                {"場面色", "GBuffer法線", "GBuffer粗度", "線形深度", "再構築線形深度"},
                {"場景顏色", "GBuffer法線", "GBuffer粗糙度", "線性深度", "重建線性深度"},
                {"场景颜色", "GBuffer法线", "GBuffer粗糙度", "线性深度", "重建线性深度"}};
            ImGui::Combo(label[ui_controller->m_language_index], &ui_model->m_view_index, items[ui_controller->m_language_index], IM_ARRAYSIZE(items[0]));
        }

        if (0 == ui_model->m_view_index)
        {
            constexpr char const *const label[4] = {
                "Clear Value##English",
                "Clear 値##Japanese",
                "Clear 值##Traditional-Chinese",
                "Clear 值##Simplified-Chinese"};
            ImGui::ColorEdit3(label[ui_controller->m_language_index], &ui_model->m_scene_color_clear_value[0]);
        }
        else if (3 == ui_model->m_view_index || 4 == ui_model->m_view_index)
        {
            constexpr char const *const label[4] = {
                "Scale##English",
                "比率##Japanese",
                "比例##Traditional-Chinese",
                "比例##Simplified-Chinese"};

            ImGui::SliderFloat(label[ui_controller->m_language_index], &ui_model->m_linear_depth_scale, 1.0F, 14.0F);
        }
    }
