...
int APIENTRY wWinMain(_In_ HINSTANCE hInstance, _In_opt_ HINSTANCE, _In_ LPWSTR lpCmdLine, _In_ int nCmdShow)
{
    ...
    while (wsi_state.m_running)
    {
        ...
        if (wsi_state.m_running && wsi_state.m_window_width > 0 && wsi_state.m_window_height > 0)
        {
            ...
            // Motion
            {
                uint32_t const animation_count = wsi_state.m_ui_model.m_asset_motion.m_animation_instances.size();
                for (uint32_t animation_index = 0U; animation_index < animation_count; ++animation_index)
                {
                    ...
                    brx_motion_animation_instance *const animation_instance = wsi_state.m_ui_model.m_asset_motion.m_animation_instances[animation_index];
                    animation_instance->step(clipped_interval_time);
                }

                assert(wsi_state.m_ui_model.m_asset_model.m_surface_group_instances.size() == wsi_state.m_ui_model.m_asset_model.m_skeleton_instances.size());
                uint32_t const surface_group_count = wsi_state.m_ui_model.m_asset_model.m_surface_group_instances.size();
                for (uint32_t surface_group_index = 0U; surface_group_index < surface_group_count; ++surface_group_index)
                {
                    brx_anari_surface_group_instance *const surface_group_instance = wsi_state.m_ui_model.m_asset_model.m_surface_group_instances[surface_group_index];
                    brx_motion_skeleton_instance *const skeleton_instance = wsi_state.m_ui_model.m_asset_model.m_skeleton_instances[surface_group_index];
                    if (wsi_state.m_ui_model.m_asset_motion.m_animation_instances.size() > surface_group_index)
                    {
                        for (uint32_t morph_target_name_index = 0U; morph_target_name_index < BRX_MOTION_MORPH_TARGET_NAME_MMD_COUNT; ++morph_target_name_index)
                        {
                            ...
                            float const morph_target_weight = wsi_state.m_ui_model.m_asset_motion.m_animation_instances[surface_group_index]->get_morph_target_weight(morph_target_name);
            ...
            wsi_state.m_anari_device->renderer_render_frame();
        }
    }
