...

		{
			...
			for (internal_mmd_mesh_section_t const &mesh_section : mesh_sections)
			{
				...
				float roughness_factor = 1.0F;
				{
					...
					if (!mesh_section.m_is_double_sided)
					{
					...
					{
						assert(0U == (raw_index_count % 3U));
						uint32_t const raw_face_count = raw_index_count / 3U;

						uint32_t const index_count = 2U * 3U * raw_face_count;
						indices.resize(index_count);
						...
						for (uint32_t raw_face_index = 0U; raw_face_index < raw_face_count; ++raw_face_index)
						{
							...
							indices[index_index_front_v0] = raw_indices[raw_index_index_v0];
							indices[index_index_front_v1] = raw_indices[raw_index_index_v1];
							indices[index_index_front_v2] = raw_indices[raw_index_index_v2];

							indices[index_index_back_v0] = raw_indices[raw_index_index_v2];
							indices[index_index_back_v1] = raw_indices[raw_index_index_v1];
							indices[index_index_back_v2] = raw_indices[raw_index_index_v0];
						}
					}
				...
				surfaces.emplace_back(std::move(vertex_positions), std::move(vertex_varyings), std::move(vertex_blendings), std::move(mesh_section_morph_target_names), std::move(morph_targets_vertex_positions), std::move(morph_targets_vertex_varyings), std::move(indices), std::move(emissive_image_url), emissive_factor, std::move(normal_image_url), normal_scale, std::move(base_color_image_url), base_color_factor, std::move(metallic_roughness_image_url), metallic_factor, roughness_factor);
			}
