...
extern bool internal_import_vrm_model(void const *data_base, size_t data_size, mcrt_vector<brx_asset_import_mesh_surface_group> &out_surface_groups)
{
    ...
    mcrt_vector<brx_asset_import_rigid_transform> animation_skeleton_joint_transforms_bind_pose_local_space;
    {
        ...
        internal_import_animation_skeleton(gltf_data, gltf_node_model_space_matrices, gltf_meshes_instance_node_indices, animation_skeleton_joint_names, animation_skeleton_joint_parent_indices, gltf_node_to_animation_skeleton_joint_map, animation_skeleton_bind_pose_local_space_matrices, animation_skeleton_bind_pose_model_space_matrices, animation_skeleton_joint_transforms_bind_pose_local_space);
                                ...
                                for (int vrmc_vrm_expression_preset_morph_target_bind_element_index = 0; vrmc_vrm_expression_preset_morph_target_bind_element_index < vrmc_vrm_expression_preset_morph_target_bind_element_count; ++vrmc_vrm_expression_preset_morph_target_bind_element_index)
                                {
                                    ...
                                    for (int vrmc_vrm_expression_preset_morph_target_bind_element_property_index = 0; vrmc_vrm_expression_preset_morph_target_bind_element_property_index < vrmc_vrm_expression_preset_morph_target_bind_element_property_count; ++vrmc_vrm_expression_preset_morph_target_bind_element_property_index)
                                    {
                                        ...
                                        else if ((6U == vrmc_vrm_expression_preset_morph_target_bind_element_property_name_length) && (0 == std::strncmp("weight", vrmc_vrm_expression_preset_morph_target_bind_element_property_name_string, 6U)))
                                        {
                                            ...
                                            assert(-1 == vrmc_vrm_expression_preset_morph_target_bind_weight);
                ...
                if ((10U == vrmc_vrm_humanoid_property_name_length) && (0 == std::strncmp("humanBones", vrmc_vrm_humanoid_property_name_string, 10U)))
                {
                    ...
                    for (int vrmc_vrm_human_bone_property_index = 0; vrmc_vrm_human_bone_property_index < vrmc_vrm_human_bone_property_count; ++vrmc_vrm_human_bone_property_index)
                    {
                        ...
                        for (int vrmc_vrm_human_bone_property_value_property_index = 0; vrmc_vrm_human_bone_property_value_property_index < vrmc_vrm_human_bone_property_value_property_count; ++vrmc_vrm_human_bone_property_value_property_index)
                        {
                            ...
                            char const *const vrmc_vrm_human_bone_property_value_property_name = json + tokens[token_index].start;
                            size_t const vrmc_vrm_human_bone_property_value_property_name_length = tokens[token_index].end - tokens[token_index].start;
                            ++token_index;
                            assert(token_index <= token_count);

                            if ((4U == vrmc_vrm_human_bone_property_value_property_name_length) && (0 == std::strncmp("node", vrmc_vrm_human_bone_property_value_property_name, 4U)))
                            {
                                if (internal_unlikely(JSMN_PRIMITIVE != tokens[token_index].type))
                                {
                                    assert(false);
                            ...
                            if ((5U == vrm_blend_shape_group_element_property_name_length) && (0 == std::strncmp("binds", vrm_blend_shape_group_element_property_name_string, 5U)))
                            {
                                ...
                                for (int vrm_blend_shape_group_element_bind_element_index = 0; vrm_blend_shape_group_element_bind_element_index < vrm_blend_shape_group_element_bind_element_count; ++vrm_blend_shape_group_element_bind_element_index)
                                {
                                    ...
                                    for (int vrm_blend_shape_group_element_bind_element_property_index = 0; vrm_blend_shape_group_element_bind_element_property_index < vrm_blend_shape_group_element_bind_element_property_count; ++vrm_blend_shape_group_element_bind_element_property_index)
                                    {
                                        ...
                                        char const *const vrm_blend_shape_group_element_bind_element_property_name = json + tokens[token_index].start;
                                        ...
                                        if ((5U == vrm_blend_shape_group_element_bind_element_property_name_length) && (0 == std::strncmp("index", vrm_blend_shape_group_element_bind_element_property_name, 5U)))
                                        {
                                            if (internal_unlikely(JSMN_PRIMITIVE != tokens[token_index].type))
                                            {
                                                assert(false);
                                        ...
                                        else if ((4U == vrm_blend_shape_group_element_bind_element_property_name_length) && (0 == std::strncmp("mesh", vrm_blend_shape_group_element_bind_element_property_name, 4U)))
                                        {
                                            if (internal_unlikely(JSMN_PRIMITIVE != tokens[token_index].type))
                                            {
                                                assert(false);
                                        ...
                                        else if ((6U == vrm_blend_shape_group_element_bind_element_property_name_length) && (0 == std::strncmp("weight", vrm_blend_shape_group_element_bind_element_property_name, 6U)))
                                        {
                                            if (internal_unlikely(JSMN_PRIMITIVE != tokens[token_index].type))
                                            {
                                                assert(false);
                ...
                if ((10U == vrm_humanoid_property_name_length) && (0 == std::strncmp("humanBones", vrm_humanoid_property_name_string, 10U)))
                {
                    ...
                    for (int vrm_human_bone_element_index = 0; vrm_human_bone_element_index < vrm_human_bone_element_count; ++vrm_human_bone_element_index)
                    {
                        ...
                        for (int vrm_human_bone_element_property_index = 0; vrm_human_bone_element_property_index < vrm_human_bone_element_property_count; ++vrm_human_bone_element_property_index)
                        {
                            ...
                            char const *const vrm_human_bone_element_property_name = json + tokens[token_index].start;
                            size_t const vrm_human_bone_element_property_name_length = tokens[token_index].end - tokens[token_index].start;
                            ++token_index;
                            assert(token_index <= token_count);

                            if ((4U == vrm_human_bone_element_property_name_length) && (0 == std::strncmp("bone", vrm_human_bone_element_property_name, 4U)))
                            {
                                if (internal_unlikely(JSMN_STRING != tokens[token_index].type))
                                {
                                    assert(false);
                            ...
                            else if ((4U == vrm_human_bone_element_property_name_length) && (0 == std::strncmp("node", vrm_human_bone_element_property_name, 4U)))
                            {
                                if (internal_unlikely(JSMN_PRIMITIVE != tokens[token_index].type))
                                {
                                    assert(false);
...
static inline void internal_import_animation_skeleton(cgltf_data const *in_gltf_data, mcrt_vector<DirectX::XMFLOAT4X4> &out_gltf_node_model_space_matrices, mcrt_vector<mcrt_set<uint32_t>> &out_gltf_meshes_instance_node_indices, mcrt_vector<BRX_ASSET_IMPORT_SKELETON_JOINT_NAME> &out_animation_skeleton_joint_names, mcrt_vector<uint32_t> &out_animation_skeleton_joint_parent_indices, mcrt_vector<uint32_t> &out_gltf_node_to_animation_skeleton_joint_map, mcrt_vector<DirectX::XMFLOAT4X4> &out_animation_skeleton_bind_pose_local_space_matrices, mcrt_vector<DirectX::XMFLOAT4X4> &out_animation_skeleton_bind_pose_model_space_matrices, mcrt_vector<brx_asset_import_rigid_transform> &out_animation_skeleton_joint_transforms_bind_pose_local_space)
{
    ...
    assert(out_animation_skeleton_joint_transforms_bind_pose_local_space.empty());
...

            {
                ...
                for (size_t gltf_node_index = 0; gltf_node_index < in_gltf_data->nodes_count; ++gltf_node_index)
                {
                    ...
                    if (gltf_node->has_matrix)
                    {
                        ...
                        gltf_node_local_space_matrices[gltf_node_index] = (*reinterpret_cast<DirectX::XMFLOAT4X4 const *>(&gltf_node->matrix));
            ...
            internal_scene_depth_first_search_traverse(
                ...
                [](cgltf_data const *in_gltf_data, cgltf_node const *in_current_gltf_node, cgltf_node const *in_parent_gltf_node, void *user_data) -> void
                {
                    ...
                    if ((gltf_current_node_index != in_gltf_skeleton_root_node_index) && (in_gltf_skeleton_node_indices.end() != in_gltf_skeleton_node_indices.find(gltf_current_node_index)))
                    {
                        ...
                        uint32_t const animation_skeleton_joint_current_index = (out_animation_skeleton_joint_parent_indices.size() - 1U);
                        assert(parent_joint_index < animation_skeleton_joint_current_index);

                        assert(BRX_ASSET_IMPORT_UINT32_INDEX_INVALID == out_gltf_node_to_animation_skeleton_joint_map[gltf_current_node_index]);
                        out_gltf_node_to_animation_skeleton_joint_map[gltf_current_node_index] = animation_skeleton_joint_current_index;

                        assert(BRX_ASSET_IMPORT_UINT32_INDEX_INVALID == out_animation_skeleton_joint_to_gltf_node_map[animation_skeleton_joint_current_index]);
                        out_animation_skeleton_joint_to_gltf_node_map[animation_skeleton_joint_current_index] = gltf_current_node_index;
    ...
    assert(out_animation_skeleton_joint_transforms_bind_pose_local_space.empty());
    {
        uint32_t const animation_skeleton_joint_count = out_animation_skeleton_bind_pose_local_space_matrices.size();

        out_animation_skeleton_joint_transforms_bind_pose_local_space.resize(animation_skeleton_joint_count);
        ...
        for (uint32_t animation_skeleton_joint_index = 0U; animation_skeleton_joint_index < animation_skeleton_joint_count; ++animation_skeleton_joint_index)
        {
            ...
            bool const directx_xm_matrix_decompose = DirectX::XMMatrixDecompose(&simd_animation_skeleton_joint_transform_bind_pose_local_space_scale, &simd_animation_skeleton_joint_transform_bind_pose_local_space_rotation, &simd_animation_skeleton_joint_transform_bind_pose_local_space_translation, DirectX::XMLoadFloat4x4(&out_animation_skeleton_bind_pose_local_space_matrices[animation_skeleton_joint_index]));
            assert(directx_xm_matrix_decompose);
...
static inline void internal_import_surface(cgltf_data const *in_gltf_data, mcrt_vector<DirectX::XMFLOAT4X4> const &in_gltf_node_model_space_matrices, mcrt_vector<mcrt_set<uint32_t>> const &in_gltf_meshes_instance_node_indices, mcrt_vector<uint32_t> const &in_gltf_node_to_animation_skeleton_joint_map, mcrt_vector<std::pair<uint32_t, internal_vrm_mesh_section_t>> &out_materials_and_mesh_sections)
{
    ...
    {
        bool lhs_double_sided;
        uint32_t lhs_emissive_factor;
        mcrt_vector<uint8_t> lhs_emissive_image_uri;
        uint64_t lhs_normal_factor;
        mcrt_vector<uint8_t> lhs_normal_image_uri;
        uint32_t lhs_base_color_factor;
        mcrt_vector<uint8_t> lhs_base_color_image_uri;
        uint32_t lhs_metallic_roughness_factor;
        mcrt_vector<uint8_t> lhs_metallic_roughness_image_uri;
        internal_import_material(gltf_materials + lhs, lhs_double_sided, lhs_emissive_image_uri, lhs_emissive_factor, lhs_normal_image_uri, lhs_normal_factor, lhs_base_color_image_uri, lhs_base_color_factor, lhs_metallic_roughness_image_uri, lhs_metallic_roughness_factor);

        bool rhs_double_sided;
        uint32_t rhs_emissive_factor;
        mcrt_vector<uint8_t> rhs_emissive_image_uri;
        uint64_t rhs_normal_factor;
        mcrt_vector<uint8_t> rhs_normal_image_uri;
        uint32_t rhs_base_color_factor;
        mcrt_vector<uint8_t> rhs_base_color_image_uri;
        uint32_t rhs_metallic_roughness_factor;
        mcrt_vector<uint8_t> rhs_metallic_roughness_image_uri;
        internal_import_material(gltf_materials + rhs, rhs_double_sided, rhs_emissive_image_uri, rhs_emissive_factor, rhs_normal_image_uri, rhs_normal_factor, rhs_base_color_image_uri, rhs_base_color_factor, rhs_metallic_roughness_image_uri, rhs_metallic_roughness_factor);

        // double sided
        if ((!lhs_double_sided) && (rhs_double_sided))
        {
            return true;
        }

        if ((!rhs_double_sided) && (lhs_double_sided))
        {
            return false;
        }

        // emissive
        if (lhs_emissive_factor < rhs_emissive_factor)
        {
            return true;
        }

        if (rhs_emissive_factor < lhs_emissive_factor)
        {
            return false;
        }

        {
            size_t const min_size = sizeof(uint8_t) * std::min(lhs_emissive_image_uri.size(), rhs_emissive_image_uri.size());
            if (min_size > 0U)
            {
                assert((!lhs_emissive_image_uri.empty()) && (!rhs_emissive_image_uri.empty()));

                int const mem_cmp = std::memcmp(lhs_emissive_image_uri.data(), rhs_emissive_image_uri.data(), min_size);
                ...
                {
                    assert(0 == mem_cmp);
                    if (lhs_emissive_image_uri.size() < rhs_emissive_image_uri.size())
                    {
                        return true;
                    }

                    if (rhs_emissive_image_uri.size() < lhs_emissive_image_uri.size())
                    {
                        return false;
                    }
                }
        ...
        // normal
        if (lhs_normal_factor < rhs_normal_factor)
        {
            return true;
        }

        if (rhs_normal_factor < lhs_normal_factor)
        {
            return false;
        }

        {
            size_t const min_size = sizeof(uint8_t) * std::min(lhs_normal_image_uri.size(), rhs_normal_image_uri.size());
            if (min_size > 0U)
            {
                assert((!lhs_normal_image_uri.empty()) && (!rhs_normal_image_uri.empty()));

                int const mem_cmp = std::memcmp(lhs_normal_image_uri.data(), rhs_normal_image_uri.data(), min_size);
                ...
                {
                    assert(0 == mem_cmp);
                    if (lhs_normal_image_uri.size() < rhs_normal_image_uri.size())
                    {
                        return true;
                    }

                    if (rhs_normal_image_uri.size() < lhs_normal_image_uri.size())
                    {
                        return false;
                    }
                }
        ...
        // base color
        if (lhs_base_color_factor < rhs_base_color_factor)
        {
            return true;
        }

        if (rhs_base_color_factor < lhs_base_color_factor)
        {
            return false;
        }

        {
            size_t const min_size = sizeof(uint8_t) * std::min(lhs_base_color_image_uri.size(), rhs_base_color_image_uri.size());
            if (min_size > 0U)
            {
                assert((!lhs_base_color_image_uri.empty()) && (!rhs_base_color_image_uri.empty()));

                int const mem_cmp = std::memcmp(lhs_base_color_image_uri.data(), rhs_base_color_image_uri.data(), min_size);
                ...
                {
                    assert(0 == mem_cmp);
                    if (lhs_base_color_image_uri.size() < rhs_base_color_image_uri.size())
                    {
                        return true;
                    }

                    if (rhs_base_color_image_uri.size() < lhs_base_color_image_uri.size())
                    {
                        return false;
                    }
                }
        ...
        // roughness metallic
        if (lhs_metallic_roughness_factor < rhs_metallic_roughness_factor)
        {
            return true;
        }

        if (rhs_metallic_roughness_factor < lhs_metallic_roughness_factor)
        {
            return false;
        }

        {
            size_t const min_size = sizeof(uint8_t) * std::min(lhs_metallic_roughness_image_uri.size(), rhs_metallic_roughness_image_uri.size());
            if (min_size > 0U)
            {
                assert((!lhs_metallic_roughness_image_uri.empty()) && (!rhs_metallic_roughness_image_uri.empty()));

                int const mem_cmp = std::memcmp(lhs_metallic_roughness_image_uri.data(), rhs_metallic_roughness_image_uri.data(), min_size);
                ...
                {
                    assert(0 == mem_cmp);
                    if (lhs_metallic_roughness_image_uri.size() < rhs_metallic_roughness_image_uri.size())
                    {
                        return true;
                    }

                    if (rhs_metallic_roughness_image_uri.size() < lhs_metallic_roughness_image_uri.size())
                    {
                        return false;
                    }
                }
