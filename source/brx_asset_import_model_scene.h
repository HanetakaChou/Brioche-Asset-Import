...
class brx_asset_import_model_surface final : public brx_asset_import_surface
{
    ...
    uint32_t m_vrm_morph_target_indices[BRX_ASSET_IMPORT_VRM_MORPH_TARGET_NAME_COUNT];
    ...
    uint32_t m_pbr_texture_incides[BRX_ASSET_IMPORT_PBR_TEXTURE_NAME_COUNT];
...
private:
    ...
    uint32_t get_vrm_morph_target_name_index(BRX_ASSET_IMPORT_VRM_MORPH_TARGET_NAME vrm_morph_target_name) const override;
    ...
    uint32_t get_pbr_texture_index(BRX_ASSET_IMPORT_PBR_TEXTURE_NAME pbr_texture_name) const override;
};

class brx_asset_import_model_group final : public brx_asset_import_group
{
    ...
    mcrt_vector<brx_asset_import_skeleton_animation_joint_transform> m_inverse_bind_pose_skeleton_joint_transforms;
    uint32_t m_vrm_skeleton_joint_indices[BRX_ASSET_IMPORT_VRM_SKELETON_JOINT_NAME_COUNT];

public:
    brx_asset_import_model_group(
...
private:
    ...
    char const* get_skeleton_joint_names(uint32_t skeleton_joint_index) const override;
    uint32_t const* get_skeleton_joint_parent_indices() const override;
    brx_asset_import_skeleton_animation_joint_transform const* get_inverse_bind_pose_skeleton_joint_transforms() const override;
    uint32_t get_vrm_skeleton_joint_index(BRX_ASSET_IMPORT_VRM_SKELETON_JOINT_NAME vrm_skeleton_joint_name) const override;
};

class brx_asset_import_model_scene final : public brx_asset_import_scene
{
    mcrt_vector<brx_asset_import_model_group> m_groups;

public:
    brx_asset_import_model_scene(mcrt_vector<brx_asset_import_model_group> &&groups);
    ~brx_asset_import_model_scene();

private:
    uint32_t get_group_count() const override;
    brx_asset_import_group const *get_group(uint32_t group_index) const override;
};
