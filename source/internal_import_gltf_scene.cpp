...
extern bool internal_import_gltf_scene(brx_asset_import_input_stream_factory *input_stream_factory, char const *file_name, mcrt_vector<brx_asset_import_model_surface_group> &out_surface_groups)
{
    ...
    mcrt_vector<brx_motion_skeleton_joint_transform> skeleton_bind_pose_joint_transforms;
...
{
    // TODO: support
    static_assert(-1 == static_cast<int32_t>(BRX_MOTION_SKELETON_JOINT_INDEX_INVALID), "");
    ...
    mcrt_vector<brx_motion_skeleton_joint_transform> node_bind_pose_transforms(static_cast<size_t>(data->nodes_count));

    uint32_t root_node_index;

    {
        mcrt_vector<uint32_t> node_parent_indices(static_cast<size_t>(data->nodes_count), BRX_MOTION_SKELETON_JOINT_INDEX_INVALID);
...

        {
            ...
            internal_scene_depth_first_search_traverse(
                ...
                [](cgltf_data const *data, cgltf_node const *current_node, cgltf_node const *parent_node, void *user_data_x, void *user_data_y, void *user_data_z, void *user_data_u, void *user_data_v, void *user_data_w, void *user_data_l, void *user_data_m, void *user_data_n) -> void
                {
                    ...
                    assert(BRX_MOTION_SKELETON_JOINT_INDEX_INVALID == node_parent_indices[cgltf_node_index(data, current_node)]);
                    node_parent_indices[cgltf_node_index(data, current_node)] = ((NULL != parent_node) ? cgltf_node_index(data, parent_node) : BRX_MOTION_SKELETON_JOINT_INDEX_INVALID);
        ...
        for (cgltf_skin const *skin : skins)
        {
            ...
            for (size_t joint_index = 0; joint_index < skin->joints_count; ++joint_index)
            {
                ...
                while (node_index_in_skeleton.end() == (found_node_index = node_index_in_skeleton.find(node_index)))
                {
                    ...
                    if (BRX_MOTION_SKELETON_JOINT_INDEX_INVALID != node_parent_indices[node_index])
                    {
        ...
        else
        {
            ...
            root_node_index = BRX_MOTION_SKELETON_JOINT_INDEX_INVALID;
        }
    }

    out_internal_node_index_to_skeleton_joint_index.assign(static_cast<size_t>(data->nodes_count), BRX_MOTION_SKELETON_JOINT_INDEX_INVALID);

    // the zero-th joint is always the root
    if (BRX_MOTION_SKELETON_JOINT_INDEX_INVALID != root_node_index)
    {
        ...
        out_skeleton_joint_parent_indices.push_back(BRX_MOTION_SKELETON_JOINT_INDEX_INVALID);
    ...
    internal_scene_depth_first_search_traverse(
        ...
        [](cgltf_data const *data, cgltf_node const *current_node, cgltf_node const *parent_node, void *user_data_x, void *user_data_y, void *user_data_z, void *user_data_u, void *user_data_v, void *user_data_w, void *user_data_l, void *user_data_m, void *user_data_n) -> void
        {
            ...
            mcrt_vector<brx_motion_skeleton_joint_transform> &out_skeleton_bind_pose_joint_transforms = *static_cast<mcrt_vector<brx_motion_skeleton_joint_transform> *>(user_data_z);
            ...
            mcrt_vector<brx_motion_skeleton_joint_transform> &node_bind_pose_transforms = *static_cast<mcrt_vector<brx_motion_skeleton_joint_transform> *>(user_data_w);
            ...
            if ((cgltf_node_index(data, current_node) != root_node_index) && (node_index_in_skeleton.end() != node_index_in_skeleton.find(cgltf_node_index(data, current_node))))
            {
...

                {
                    ...
                    assert(BRX_MOTION_SKELETON_JOINT_INDEX_INVALID != parent_joint_index);
                    ...
                    assert(BRX_MOTION_SKELETON_JOINT_INDEX_INVALID == out_internal_node_index_to_skeleton_joint_index[cgltf_node_index(data, current_node)]);
    ...
    {
        assert((BRX_MOTION_SKELETON_JOINT_INDEX_INVALID == out_skeleton_joint_parent_indices[skeleton_joint_index]) || (out_skeleton_joint_parent_indices[skeleton_joint_index] < skeleton_joint_index));
    }
#endif

    assert(out_skeleton_joint_names.size() == ((BRX_MOTION_SKELETON_JOINT_INDEX_INVALID != root_node_index) ? node_index_in_skeleton.size() : (node_index_in_skeleton.size() + 1U)));
    assert(out_skeleton_joint_parent_indices.size() == ((BRX_MOTION_SKELETON_JOINT_INDEX_INVALID != root_node_index) ? node_index_in_skeleton.size() : (node_index_in_skeleton.size() + 1U)));
    assert(out_skeleton_bind_pose_joint_transforms.size() == ((BRX_MOTION_SKELETON_JOINT_INDEX_INVALID != root_node_index) ? node_index_in_skeleton.size() : (node_index_in_skeleton.size() + 1U)));
}
